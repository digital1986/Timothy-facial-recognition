...
class SensorApp:
    ...
    def animate(self):
        radius = 10 + 40 * abs(math.sin(self.angle))
        self.angle += 0.1
        x0 = 100 - radius
        y0 = 100 - radius
        x1 = 100 + radius
        y1 = 100 + radius
        self.canvas.coords(self.sonar, x0, y0, x1, y1)

        if GPIO.input(BUZZER):
            color = "#ff5555" if int(time.time() * 2) % 2 == 0 else "#ffaaaa"
        else:
            color = "cyan"
        self.canvas.itemconfig(self.sonar, outline=color)
...
def on_close():
    GPIO.cleanup()
    if hasattr(app, 'face_running') and app.face_running:
